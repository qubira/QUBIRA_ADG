...
export function render() {
  ...
  Promise.all([
    ...
    api.get('/projects', { status: 'finished_by_ti' }),
  ]).then(([stats, allProjects, activities, toReview]) => {
    ...
    <div class="p-6 max-w-7xl mx-auto">
      ...
      ${toReview.length > 0 ? `
      <div class="card p-5 mb-6 border-l-4 border-l-purple-500 bg-purple-50/40">
        <div class="flex items-center gap-2 mb-3">
          ${icon('notifications_active', 20)}
          <h2 class="font-semibold text-purple-900">
            ${toReview.length} proyecto${toReview.length !== 1 ? 's' : ''} finalizado${toReview.length !== 1 ? 's' : ''} por TI — esperando tu revisión
          </h2>
        </div>
        <div class="space-y-2">
          ${toReview.map(p => `
            <a href="#/projects/${p.id}" class="flex items-center justify-between px-3 py-2 rounded-lg bg-white hover:bg-purple-50 transition-colors">
              <div>
                <p class="text-sm font-medium text-gray-900">${esc(p.name)}</p>
                <p class="text-xs text-gray-500">${esc(p.code)} · ${esc(p.client)}</p>
              </div>
              <span class="text-xs text-purple-600 font-medium flex items-center gap-1">Revisar ${icon('chevron_right', 14)}</span>
            </a>`).join('')}
        </div>
      </div>` : ''}
      ...
      <div class="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        ...
        ${statCard(icon('task_alt', 22), 'Por revisar', stats?.finishedByTi ?? '—', 'bg-purple-500')}
